...
then
    tmp="$(mktemp)"
    echo -e "// Copyright $year Dolphin Triforce Emulator Project\n// Licensed under GPLv2+\n// Refer to the license.txt file included.\n"|cat - "$filename" >"$tmp"
    mv "$tmp" "$filename"
else
    sed -ri "1 s|// Copyright ([0-9-]+) Dolphin Triforce Emulator Project|// Copyright $year Dolphin Triforce Emulator Project|" "$filename"
fi
